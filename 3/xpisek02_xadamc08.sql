...
ALTER TABLE MEMBER_TABLE ADD CONSTRAINT FAMILY_FK_MEM_TAB FOREIGN KEY (FAMILY_ID) REFERENCES FAMILY (FAMILY_ID) ON DELETE CASCADE;

ALTER TABLE FAMILY ADD CONSTRAINT DON_FK_FAM FOREIGN KEY (DON_ID) REFERENCES DON (DON_ID) ON DELETE CASCADE;

ALTER TABLE OPERATION ADD CONSTRAINT OWNING_FAMILY_FK_OP FOREIGN KEY (OWNING_FAMILY) REFERENCES FAMILY (FAMILY_ID) ON DELETE CASCADE;
...
GROUP BY
    AUTHORIZATION
HAVING
    AUTHORIZATION LIKE 'Velkej čavo';
...
WHERE
    EXISTS(
        ...
        FROM
            TERRITORY
        WHERE
            MEETING.TERRITORY_ID = TERRITORY.GPS
            AND AREA > 2000
    );
